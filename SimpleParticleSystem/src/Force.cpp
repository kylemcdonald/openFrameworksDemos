...
ForceLimit::ForceLimit(float limit)
	:limit(limit) {
}

ofVec2f ForceLimit::getForce(Particle& particle) const {
	const ofVec2f& velocity = particle.getVelocity();
	float speed = velocity.length();
	if(speed > limit) {
		return (limit - speed) * velocity;
	} else {
		return ofVec2f(0);
	}
}
